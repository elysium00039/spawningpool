...
class Daimma_Admin
{
    ...
    static private function ac_initialize_options()
    {
        ...
        //general
        add_option($shared->get('slug') . "_import_post_type", "post");
        add_option($shared->get('slug') . "_import_menu_required_capability", "edit_posts");
        add_option($shared->get('slug') . "_markdown_parser", "parsedown");
        add_option($shared->get('slug') . "_cebe_markdown_html5", "0");
        add_option($shared->get('slug') . "_cebe_markdown_keep_list_start_number", "0");
        add_option($shared->get('slug') . "_cebe_markdown_enable_new_lines", "0");
	...
	public function change_footer_text() {
		...
		if ( $screen->id == $this->screen_id_import or
		     $screen->id == $this->screen_id_help or
		     $screen->id == $this->screen_id_pro_version or
		     $screen->id == $this->screen_id_options ) {

			echo '<a target="_blank" href="http://wordpress.org/support/plugin/import-markdown#postform">' . esc_attr__( 'Contact Support',
					'import-markdown' ) . '</a> | ' .
			     '<a target="_blank" href="https://translate.wordpress.org/projects/wp-plugins/import-markdown/">' . esc_attr__( 'Translate',
					'import-markdown' ) . '</a> | ' .
			     str_replace(
				     [ '[stars]', '[wp.org]' ],
				     [
					     '<a target="_blank" href="https://wordpress.org/support/plugin/import-markdown/reviews/?filter=5">&#9733;&#9733;&#9733;&#9733;&#9733;</a>',
					     '<a target="_blank" href="http://wordpress.org/plugins/import-markdown/" >wordpress.org</a>'
				     ],
				     __( 'Add your [stars] on [wp.org] to spread the love.', 'import-markdown' )
			     );
